...
async def sportsdb_player(session, player_name: str) -> str:
    ...
    async with session.get(url, headers=HEADERS) as r:
        ...
        if not players:
            raise Exception("no player found in SportsDB")
        photo = players[0].get('strThumb') or players[0].get('strCutout') or players[0].get('strImage')
        if not photo:
            raise Exception("no photo in SportsDB")
        return photo

async def wiki_search_summary(session, query: str) -> str:
    """Wikipedia search → najde správný anglický článek → vrátí URL obrázku."""
    search_url = (f"https://en.wikipedia.org/w/api.php?action=query&list=search"
                  f"&srsearch={quote(query)}&format=json&origin=*&srlimit=1")
    async with session.get(search_url, headers=HEADERS) as r:
        if r.status != 200:
            raise Exception(f"HTTP {r.status}")
        d = await r.json()
        ...
        title = results[0]['title']
    return await wiki_summary_img(session, title)
...
async def download_player(session, wiki_title: str, redownload: bool):
    ...
    async with SEM:
        img_url = None
        for attempt in [
            lambda: sportsdb_player(session, wiki_title),
            lambda: wiki_summary_img(session, wiki_title),
            lambda: wiki_pageimages(session, wiki_title),
            lambda: wiki_search_summary(session, wiki_title + ' footballer'),
        ]:
            try:
                img_url = await attempt()
                break
